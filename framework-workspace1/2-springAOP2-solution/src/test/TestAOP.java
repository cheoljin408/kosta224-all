...
public class TestAOP {
	 ...
	 * - 2안) Component or Library를 이용해 각 core에서 import해서 사용 -> 위의 첫번째 안보다는 효율적
	 *   Log4j Component를 이용해 개발 -> 별도의 프로젝트에서 단위 테스트( 2-springAOP2-ex2-log4j-test )
	 *   
	 *   적용단계 1. maven dependency(library) Log4j
	 *   	   2. 설정 파일 등록log4j2.xml
	 *   	   3. core의 대상 메소드에 로길 기능을 적용 (info level로 로깅)
	 *    
	 *   횡단관심사(cross cutting concern)인 로깅 작업을 log4j 컴포넌트를 이용해
	 *   각 core 에서는 import 후 logger 기능 코드 작업을 진행
	 *   -> 1안에 비해 작업을 많이 줄였지만 여전히 반복적인 작업은 불가피하다
	 *   
	 * - 3안) AOP 를 적용하면 기존 CORE에 변경없이 횡단관심사(cross cutting concern)인 로깅 작업을 반영할 수 있다
	 * 		 -> 2-springAOP2-solution에서 AOP 적용하여 효율적으로 개발하고 운영하도록 한다
	 * 
	 *   설계: Core Concern(회원, 상품 관리의 검색 계열 메소드)
	 *   	  Cross Cutting Concern(검색 계열의 info level logging -> 실행시간 서비스메소드명을 report.log 파일에 로깅)
	 *   
	 *   구현단계
