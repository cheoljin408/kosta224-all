/*
 	...
 	Inline View: SQL 문장에서 from 절에 사용되는 SubQuery
 				 from 절에 버스쿼리로 select 되는 조회 결과를 테이블처럼 사용
 				 ex) select 컬럼, 컬럼
 				 	 from (
 				 	 	서브쿼리
 				 	 ) 별칭
...
-- Inline View(from 절에 사용되는 Sub Query)를 사용하면 된다
select rnum, no, title
from (
	select row_number() over(order by no desc) as rnum, no, title from player
) where rnum <= 3;

-- 1 page에서 3곡을 보여주려고 하고 2 page에 해당하는 3곡을 보여주기 위해 rnum이 4이상 6이하인 곡을 조회
select rnum, no, title
from (
	select row_number() over(order by no desc) as rnum, no, title from player
