/*
	...
	1. Inner Join - 일반적으로 조인이라고 하는 SQL을 의미
					...
					2) Non Equi Join - 조인 대상 테이블 간의 상응하는 컬럼정보가 없을 때 사용하는 Join
										테이블 간의 컬럼 값들이 정확하기 일치하지 않을 때 사용하는 Join
										(사원 테이블과 월급 등급 테이블간의 일치하는 컬럼은 없지만 사원의 월급 등급을
										알기 위해 월급 등급 테이블의 hisal, losal과 사원 테이블의 sal을 비교)
										
	2. Outer Join - JOIN 조건에 부합되지 않더라고 모든행을 반환
					(부서 테이블과 사원테이블 조인시 Inner Join은 사원이 없는 부서는 제외
					Outer Join은 사원이 없는 부서라도 모두 조회)
	
	3. Self Join - 동일 테이블 상에서의 조인(사원 테이블의 매니저 컬럼의 정보는 또 다른 사원 정보이다. 이를 이용해
										사원의 매니저의 사원번호, 사원며어 등을 조회할 때 Self Join을 사용)
*/
...
-- sql				hisal, losal
-- 두 테이블 간의 연관성은 사원 테이블의 sal과 월급등급 테이블의 hisal 최고액, losal 최저액 사이에 있다
...
*/
select distinct deptno from dept; -- 10 20 30 40 부서가 존재
...
/*
	3. Self Join: 테이블 자체에서 조인
				  ...
				  사례 - 사원 테이블의 매니저 번호는 또 다른 사원의 사원번호이다
				  		하나의 사원 테이블에서 사원정보와 그 사원의 매니저 정보를 함께 조회하고자 할 때
...
-- 사원의 사원번호 empno, 사원명 ename, 매니저번호 mgr, 매니저명(다른 사원명)을 조회하고자 한다 -> 이때 self join을 이용
select e.empno, e.ename, e.mgr, m.empno, m.ename
from emp e, emp m
where e.mgr = m.empno and e.empno = 7902
...
select * from emp; -- 14명의 사원

-- Outer Join: 위의 Inner join시 결과행은 13명이 조회된다. 이유는 mgr이 없는 king 사원이
-- 제외되었기 때문
-- 매니저가 없는 사원까지 모두 조회하고자 할 때ㅑ Outer Join을 사용한다
...
-- join sql 연습
-- empno 가 7369인 사원의 사원명 ename, 부서명 dname, 월급등급 grade, 매니저명 mgr의 ename을 조회
...
-- 사원의 사원명 ename, 부서명 dname, 월급등급 salgrade, 매니저명 mgr의 ename을 조회
-- 매니저가 없는 사원꺼지 모두 조회한다(Outer join을 이용)
select e.ename, d.dname, s.grade, m.ename as mgr_name
from emp e, dept d, salgrade s, emp m
where e.deptno = d.deptno
and e.sal >= s.losal
