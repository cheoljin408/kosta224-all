 ...
 * 시스템 상에서 단 하나 존재하면 되므로 Singleton Design Pattern 을 적용
 * 
 ...
 * 실제 어플리케이션측(사용하는 측)에서는 개별 DBCP 구현체들을 보고 사용하는 것이 아니라 
 * DataSource Interface의 abstract method를 이용해 개발하게 된다
 * 이후 DBCP 구현체가 변경되어도 이를 사용하는 어플리케이션 측에서는 영향이 최소화된다 ( 결합도가 느슨해 진다 )
 */
